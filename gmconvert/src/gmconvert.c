 ...
 char **argv;
{
 ...
 char  ovfile[MAX_FILENAME];
 FILE  *ptr_file, *fp_ovfile;
 ...
 igmmfile[0] = igmmfile2[0] = ovfile[0] = '\0';    
 ...
 */
 ...
 if (strcmp(MODE,"GcmpV")==0){
   ...
   */
   ...
   // calculate linear correlation terms
   // ivox = A + B * ovox;
   // ovox = ( ivox - A ) / B
   ...
   for (x=0;x<ivox.N[0];++x){
    for (y=0;y<ivox.N[1];++y){
      for (z=0;z<ivox.N[2];++z){
       ovox_ave += ovox.dat[x][y][z];
       ivox_ave += ivox.dat[x][y][z]; 
       norm += 1.0;
      }
    }
   }
   ...
   for (x=0;x<ivox.N[0];++x){
    for (y=0;y<ivox.N[1];++y){
      for (z=0;z<ivox.N[2];++z){
        Bn += ( ovox.dat[x][y][z] - ovox_ave ) * ( ivox.dat[x][y][z] - ivox_ave );
        Bd += ( ovox.dat[x][y][z] - ovox_ave ) * ( ovox.dat[x][y][z] - ovox_ave );
      }
    }
   }
   // final calculation
   double B = Bn / Bd;
   double A = ivox_ave - B * ovox_ave;  

   // now rescale ovox
   for (x=0;x<ivox.N[0];++x){
    for (y=0;y<ivox.N[1];++y){
      for (z=0;z<ivox.N[2];++z){
          ovox.dat[x][y][z] = A + B * ovox.dat[x][y][z];
      }
    }
   }
   ...
   printf("Correlation Coefficients ivox = A + B * ovox\n");
   printf("   A : %lf\n", A);
   printf("   B : %lf\n", B);
      ...
      for (x=0;x<ivox.N[0];++x){
        ...
        for (y=0;y<ivox.N[1];++y){
          ...
          for (z=0;z<ivox.N[2];++z){
           ...
           isumDensity += ovox_GMM.dat[x][y][z] * ( ivox.dat[x][y][z] - A ) / B * dx * dy * dz;
           osumDensity += ovox_GMM.dat[x][y][z] * ( ovox.dat[x][y][z] - A ) / B * dx * dy * dz;
          }
        }
      }
      // print to file
